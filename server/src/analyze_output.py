...
from generality_pruning import prune_by_generality
...
COLORS = [mcolors.CSS4_COLORS[x] for x in ('blue', 'red', 'green',#'orange',
    'magenta', 'black', 'brown', 'blueviolet')]
MARKERS = ["o", "x", "s", "v", "^", "*", "o", "x"]
LINESTYLES = ['solid', 'dotted', 'dashed', 'dashdot', (0, (3, 5, 1, 5)), 'dotted','dashed', (0, (3, 1, 1, 1, 1, 1))]

matplotlib.rcParams['pdf.fonttype'] = 42
matplotlib.rcParams['ps.fonttype'] = 42

#path = '/usr/share/fonts/type1/texlive-fonts-recommended/bchri8a.pfb'
...
#    "font.sans-serif": ['STIXSizeTwoSym']
#})
...
def calculate_metrics_by_time_top_k_all_scores(result_path, full_result_path, k=10, time_bin_size_in_seconds=100,
                                               ...
                                               score_recall_threshold=None, score_name_subset=None, max_atoms=MAX_ATOMS):
    ...
    for i, score_name in enumerate(scores):
        if score_name not in full_res_df.columns:
            ...
            del score_name_to_stats[score_name]
            continue
...
def combine_result_files(res_file_dict, metric_name, reference_file, order,
    ...
    if metric_name == 'Inverted pvalue':
       plt.legend(fontsize=base_font_size-4, borderpad=0.2, loc='lower right')
...
def find_time_until_specific_score_recall(result_path, full_result_path, threshold=0.95, score_name_subset=None, k=100, max_atoms=MAX_ATOMS):
    score_name_to_stats = calculate_metrics_by_time_top_k_all_scores(
        ...
        score_name_subset=score_name_subset, max_atoms=max_atoms)
...
def sort_list_of_atoms_by_attrs(list_of_atoms):
    return sorted(list_of_atoms, key=lambda atom: atom.split(":")[0])
...
def analyze_oreo_output_file(file_path, full_res_df, should_prune_by_generality=True):
    # 1. filter by generality
    df = pd.read_csv(file_path)
    df['Counter argument'] = df['Counter argument'].apply(eval)
    if should_prune_by_generality:
        df = prune_by_generality(df, max_atoms=-1, is_oreo_format=True)
        b = len(df)
        df = df[df['keep_by_generality']]
        print(f"Generality removed {b-len(df)} preds")
    else:
        df['Attr_set_size'] = df['Counter argument'].apply(len)
    # removing all attribute sets larger than 2 (because they are not in our result)
    b = len(df)
    df = df[df['Attr_set_size'] <= 2]
    print(f"Max atoms=2 removed {b - len(df)} preds")
    # Processing to our format of result file
    df['Counter argument'] = df['Counter argument'].apply(sort_list_of_atoms_by_attrs)
    df['Attr1'] = df['Counter argument'].apply(lambda x: x[0].split(":")[0])
    df['Value1'] = df['Counter argument'].apply(lambda x: x[0].split(":")[1])
    df['Attr2'] = df['Counter argument'].apply(lambda x: x[1].split(":")[0] if len(x) > 1 else None)
    df['Value2'] = df['Counter argument'].apply(lambda x: x[1].split(":")[1] if len(x) > 1 else None)
    #df['Value1'] = df['Value1'].apply(safe_eval)
    #df['Value2'] = df['Value2'].apply(safe_eval)
    # 2. merge with scores from reference df
    ref = pd.read_csv(full_res_df)
    # TODO: this might be a problem with missing values and ints vs. floats
    fields = ['Attr1', 'Value1', 'Attr2', 'Value2', 'Normalized_MI', 'Normalized Anova F Stat',
              'Coverage', 'Cosine Similarity', 'Inverted pvalue', 'Metrics Average']
    m = df.merge(ref[[c for c in fields if c in ref.columns]],
                 on=['Attr1', 'Value1', 'Attr2', 'Value2'], how='left')
    return m
