...
HYPDB_ENABLE = False
if HYPDB_ENABLE:
    from hypdb_compare import compute_covariates_and_responsibility
...
class CherryPicker(object):
    def __init__(self, exclude_list, numeric_list, grp_attr, target_attr, compare_list, MI_dict,
                 ...
                 reference=None, topk=100, progressive_output_file=None, where=None, max_atoms=MAX_ATOMS,
                 db_name=None):
        ...
        self.engine = SQLEngineSingleton(db_name)
        ...
        self.progressive_output_file = progressive_output_file
        self.where = where
    ...
    def add_metrics_to_result_dataframe(self, result_df, column_tuple, calc_on_the_spot=False):
        ...
        num_attrs = [c for c in result_df.columns if c.startswith('Attr')]
        ...
        agg_field_name = self.agg_type
        if agg_field_name == 'count':
            agg_field_name = 'N'
        result_df['AggDiff'] = result_df[f'{agg_field_name}2'] - result_df[f'{agg_field_name}1']
    ...
    def cherrypick_by_attributes(self, sorted_column_tuples, sample_size=None, add_metrics=True, stop_at_k=None,
                                 ...
                                 min_group_size_in_results=MIN_GROUP_SIZE_SQL):
        print(f"Cherrypick_by_attributes: min_group_size_in_results: {min_group_size_in_results}.")
        ...
        base_args = {"grp_attr": self.grp_attr, "value1": self.compare_list[1], "value2": self.compare_list[2],
                     "table_name": table_name, "where_string": self.where}
        if self.agg_type == 'count':
            runner = QueryRunnerCountDiff(self.start_time, self.db_name)
        elif self.agg_type == 'mean':
            compute_stat_sig = DF_PVALUE_STRING in self.metric_subset
            runner = QueryRunnerMeanDiff(self.start_time, self.db_name, compute_stat_sig)
            base_args["target_attr"] = self.target_attr
            base_args["min_group_size"] = min_group_size_in_results
        elif self.agg_type == 'median':
            compute_stat_sig = DF_PVALUE_STRING in self.metric_subset
            runner = QueryRunnerMedianDiff(self.start_time, self.db_name, compute_stat_sig)
            base_args["target_attr"] = self.target_attr
            base_args["min_group_size"] = min_group_size_in_results
        else:
        ...
        for column_tuple in tqdm(sorted_column_tuples):
            ...
            agg_fields = {'mean': ['mean1', 'N1', 'mean2', 'N2'],
                          'median': ['median1', 'N1', 'median2', 'N2'],
                          'count': ['count1', 'N1', 'count2', 'N2']}
            desired_order = ['Attr1', 'Value1', 'Attr2', 'Value2'] + agg_fields[self.agg_type] + [
                'AggDiff',
                'statistical_significance_stat',
                'pvalue', 'Time', 'MI',
                'Normalized_MI', 'Anova_F_Stat', 'Anova_Pvalue',
                'Normalized Anova F Stat', 'Attr1_str', 'Value1_str', 'Attr2_str',
                'Value2_str', 'Cosine Similarity', 'Inverted pvalue', 'Coverage',
                'Metrics Average']
            attr_tuple_to_num_preds[column_tuple] = len(result_df)
            if len(result_df) > 0:
                if 'Attr2' not in result_df.columns and self.max_atoms > 1:
                    result_df['Attr2'] = None
                    result_df['Value2'] = None
                if self.agg_type == 'count':
                    result_df["statistical_significance_stat"] = None
                    result_df["pvalue"] = None
                    result_df["Inverted pvalue"] = None
                    result_df["count1"] = result_df["N1"]
                    result_df["count2"] = result_df["N2"]
                if add_metrics:
                ...
                write_header = not os.path.exists(self.progressive_output_file)
                result_df.to_csv(self.progressive_output_file, mode='a', header=write_header)
    ...
    def cherrypick_by_predicates(self, predicate_order):
        ...
        if self.agg_type == 'mean':
            runner = QueryRunnerMeanDiffPredLevel(self.start_time, self.db_name)
            # sql_string = open("sql_queries/mean_query_single_pred_case_when.sql", "r").read()
            # res_df_column_names = ['Value', 'mean1', 'N1', 's1', 'mean2', 'N2', 's2']
        elif self.agg_type == 'median':
            runner = QueryRunnerMedianDiffPredLevel(self.start_time, self.db_name)
        else:
    ...
    def sample_guided_cherrypicking_by_attribute(self, sample_size, columns_list):
        ...
        if len(res_df) > 0:
            res_df['Attr_tuple'] = res_df.apply(lambda row: unite_attr_names_to_tuple_field(row, self.max_atoms), axis=1)
...
def process_atom_combination(args):
    ...
    attr_tuple, df, attr_to_num_values, already_processed, target_attr, bucket_objects, output_path = args
    ...
    emb_sim = 0
    if max_group_size < 5:  # small optimization. Groups under 5 are never statistically significant
        with open(output_path, "a") as out:
            result_str = f'"{tuple(sorted(attr_tuple))}",{None},{None},{None},{emb_sim},{min_value_count},{max_group_size},{num_over_100},{num_over_500}\n'
            out.write(result_str)
        # return tuple(sorted(attr_tuple)), None, None, None, min_value_count, max_group_size, num_over_100, num_over_500
    try:
    ...
    with open(output_path, "a") as out:
        result_str = f'"{tuple(sorted(attr_tuple))}",{f_stat},{anova_pvalue},{mi_res},{emb_sim},{min_value_count},{max_group_size},{num_over_100},{num_over_500}\n'
        out.write(result_str)
...
def progressive_metrics_calc(atom_combinations, target_attr, max_atoms=1, translation_dict={}, bucket_objects={},
                             data_path=DATA_PATH, dataframe_path=DATAFRAME_PATH, subset=None):
    output_path = os.path.join(data_path, f"metrics_{max_atoms}atoms_{target_attr}.csv")
    if subset is not None:
        # This only changes atom_combinations inside the function and not its outside copy.
        atom_combinations = [a for a in atom_combinations if len(set(a).intersection(set(subset))) > 0]
    if os.path.exists(output_path):
        ...
        missing = set(atom_combinations).difference(already_processed)
        print(f"missing metrics for {len(missing)} combinations: {missing}")
        if len(missing) == 0:
    ...
    args_list = [(atom_combination, df, attr_to_num_values, already_processed, target_attr, bucket_objects, output_path)
    ...
    df['cosine_sim'] = df['attr_tuple'].progress_apply(lambda x: semantic_sim.calc_cosine_sim_attr_level(eval(x)))
...
def create_count_std_dictionary(atom_combinations, bucket_dict, start_time, db_name):
    std_dict = {}
    runner = QueryRunnerCountSTD(start_time=start_time, db_name=db_name)
...
def setup_cherrypicking(df, exclude_list, is_numeric, trans_dict, start_time, do_regression=False,
                        precompute_metrics=True, multi_atom_combinations=None, compute_hypdb=False,
                        target_attr=TARGET_ATTR, grp_attr=GRP_ATTR, compare_list=COMPARE_LIST, string_cols=STRING_COLS,
    ...
    for attr_name in is_numeric:
        bucket = Bucket.from_attr_name(attr_name, db_name)
    ...
    if precompute_metrics:
        Anova_dict, MI_dict, emb_sim_dict, min_value_count_dict, max_group_size_dict, num_groups_over100, num_groups_over500 = progressive_metrics_calc(
            ...
            bucket_objects=bucket_objects, data_path=data_path, dataframe_path=dataframe_path)
    std_dict = None
    if agg_type == 'count' and precompute_metrics and 'Count STD' in METRICS_SUBSET:
        # TODO: test 2-atoms
        std_dict = create_count_std_dictionary(multi_atom_combinations, bucket_objects, start_time, db_name)
    ...
    hypdb_dict = None
    if compute_hypdb:
        potential = [c for c in df.columns if c not in exclude_list]
        hypdb_dict = compute_covariates_and_responsibility(df, grp_attr, compare_list[1], compare_list[2], target_attr, potential)
        print(hypdb_dict)
    ...
    return Anova_dict, MI_dict, emb_sim_dict, std_dict, reg_weight_dict, multi_atom_combinations, bucket_objects, num_groups_over100, num_groups_over500, hypdb_dict


def sort_attr_tuples(attr_tuples, sort_by, Anova_dict, MI_dict, emb_sim_dict, std_dict, reg_weight_dict,
                     num_large_groups, hypdb_dict, iter_index=1):
    if sort_by == DF_ANOVA_PVALUE_STRING:
        ...
        sorted_by = sorted(attr_tuples, key=lambda cols: sum([hypdb_dict[col] for col in cols if col in hypdb_dict]), reverse=True)
...
def run_cherrypicking(df, exclude_list, is_numeric, trans_dict, metrics_subset,
                      ...
                      reference=None, progressive_output_path=None, where=WHERE, string_cols=STRING_COLS,
                      max_atoms=MAX_ATOMS, db_name=DATABASE_NAME, data_path=DATA_PATH, dataframe_path=DATAFRAME_PATH,
                      min_group_size_in_results=MIN_GROUP_SIZE_SQL):
    if progressive_output_path is None:
        progressive_output_path = output_path
    # incorporate all the possible running variations.
    start = time.time()
    preprocessing_time = 0
    init_res = setup_cherrypicking(
        df, exclude_list, is_numeric, trans_dict, start_time,
        do_regression=(sort_by in ('REGRESSION', 'ALL_TOP_K_MERGED', 'ALL_TOP_K_SERIAL')), compute_hypdb=(sort_by == 'HYPDB'),
        target_attr=target_attr, grp_attr=grp_attr, compare_list=compare_list, string_cols=string_cols,
        agg_type=agg_type, max_atoms=max_atoms, db_name=db_name, data_path=data_path, dataframe_path=dataframe_path
    )
    if init_res is None:
        return None
    Anova_dict, MI_dict, emb_sim_dict, std_dict, reg_weight_dict, multi_atom_combinations, bucket_objects, num_groups_over100, num_groups_over500, hdb_dict = init_res
    ##################################
    ...
    if USE_SQL:
        cherry_picker = CherryPicker(exclude_list=exclude_list, numeric_list=is_numeric,
                                     ...
                                     dataset_size=get_DB_size(main_table, db_name, target_attr), bucket_dict=bucket_objects,
                                     agg_type=agg_type,
                                     ...
                                     progressive_output_file=progressive_output_path, where=where, max_atoms=max_atoms,
                                     db_name=db_name)
        if sort_by == 'ALL_TOP_K_MERGED':
            ...
            merged_sorted = combine_top_k_from_pre_known_metrics_merged(
                ...
                num_groups_over100, num_groups_over500, hdb_dict)
            preprocessing_time += (time.time() - start)
            print(f"preprocessing time: {preprocessing_time}")
            result_df = cherry_picker.cherrypick_by_attributes(merged_sorted, min_group_size_in_results=min_group_size_in_results)
            ...
            sorted_by = sort_attr_tuples(multi_atom_combinations, sort_by, Anova_dict, MI_dict, emb_sim_dict, std_dict,
                                         reg_weight_dict, num_groups_over100, hdb_dict)
            preprocessing_time += (time.time() - start)
            ...
            sorted_by = sort_attr_tuples(multi_atom_combinations, sort_by, Anova_dict, MI_dict, emb_sim_dict, std_dict,
                                         reg_weight_dict, num_groups_over100, hdb_dict, shuffle_iter_index)
            preprocessing_time += (time.time() - start)
            result_df = cherry_picker.cherrypick_by_attributes(sorted_by)
    else:  # not using SQL
        print("Not using SQL is not supported")
        sys.exit()
    #     sorted_by = sort_attr_tuples(multi_atom_combinations, sort_by, Anova_dict, MI_dict, emb_sim_dict, std_dict,
    #                                  reg_weight_dict, num_groups_over100, hdb_dict)
...
def shuffle_and_save(df, exclude_list, is_numeric, trans_dict, start_time, iters=3):
    ...
    Anova_dict, MI_dict, emb_sim_dict, std_dict, reg_weight_dict, multi_atom_combinations, bucket_objects, num_groups_over100, num_groups_over500, hdb_dict = setup_cherrypicking(
        df, exclude_list, is_numeric, trans_dict, start_time,
        do_regression=False, compute_hypdb=False)
...
def combine_top_k_from_pre_known_metrics(cherry_picker, top_k,
    ...
    for metric in metrics:
        ...
        sorted_by = sort_attr_tuples(multi_atom_combinations, metric, Anova_dict, MI_dict, emb_sim_dict, std_dict,
                                     reg_weight_dict, num_groups_over100, hypdb_dict={})
...
def combine_top_k_from_pre_known_metrics_merged(
        Anova_dict, MI_dict, emb_sim_dict, std_dict, reg_weight_dict,
        multi_atom_combinations, num_groups_over100, num_groups_over500, hdb_dict):
    start = time.time()
...
def regression_for_feature_weight(df, target_attr, group_attr, group_value_pair, exclude, categoricals,
    ...
    if len(string_cols) > 0:
        ...
        encoder.fit(df_local, df_local[target_attr])
        g1 = df_local[df_local[group_attr] == group_value_pair[0]]
        g2 = df_local[df_local[group_attr] == group_value_pair[1]]
        if len(g1) == 0 or len(g2) == 0:
            print("Regression weighting meaningless - one group has only null values in the target attribute.")
            return {a: 0 for a in attrs}
        df_g1 = encoder.transform(g1)
        df_g2 = encoder.transform(g2)
    else:
...
def num_tuples_vs_time_for_full_run(sample_sizes, df, exclude_list, is_numeric, trans_dict, methods, stop_at_recall=False):
    out_dir = os.path.join(OUTPUT_DIR, "db_size_sensitivity")
    for size in sample_sizes:
        ...
        reference = None
        ref_path = os.path.join(out_dir, f"{DATABASE_NAME}_{AGG_TYPE}_{MAX_ATOMS}atoms_{QUERY_DESC}_original_order_guided_{size}_tuples.csv")
        if os.path.exists(ref_path):
            reference = pd.read_csv(ref_path, index_col=0)
...
def num_columns_vs_time_for_full_run(sample_sizes, df, exclude_list, is_numeric, trans_dict, methods, iterations=1,
    ...
    out_dir = os.path.join(OUTPUT_DIR, "db_width_sensitivity")
    for size in sample_sizes:
        for i in range(iterations):
            ...
            reference = None
            ref_path = os.path.join(out_dir, f"{DATABASE_NAME}_{AGG_TYPE}_{MAX_ATOMS}atoms_{QUERY_DESC}_original_order_guided_{size}cols_iter0.csv")
            if os.path.exists(ref_path):
                reference = pd.read_csv(ref_path, index_col=0)
            for method in methods:
                start = time.time()
...
def num_atoms_vs_time_for_full_run(num_atoms, df, exclude_list, is_numeric, trans_dict, methods, stop_at_recall=True):
    out_dir = os.path.join(OUTPUT_DIR, "m_experiment")
    for m in num_atoms:
        reference = None
        ref_path = os.path.join(out_dir, f"{DATABASE_NAME}_{AGG_TYPE}_{m}atoms_{QUERY_DESC}_original_order_guided.csv")
        if os.path.exists(ref_path):
            reference = pd.read_csv(ref_path, index_col=0)
        for method in methods:
            print(f"running: method {method} with {m} atoms")
            start = time.time()
            TIME_SQL_QUERIES["x"] = datetime.timedelta(seconds=0)
            iter_index = None
            method_sample = None
            ...
            output_path = os.path.join(
                out_dir,
                f"{DATABASE_NAME}_{AGG_TYPE}_{m}atoms_{QUERY_DESC}_{method.replace(':', '')}_guided.csv")
            res_df, preprocessing_time = run_cherrypicking(
                df, exclude_list, is_numeric, trans_dict, METRICS_SUBSET,
                output_path, 'my_table', sort_by, method_sample, start,
                target_attr=TARGET_ATTR, grp_attr=GRP_ATTR, compare_list=COMPARE_LIST, agg_type=AGG_TYPE,
                shuffle_iter_index=iter_index,
                reference=reference, max_atoms=m)
            if method == 'original_order' and stop_at_recall:
                print("Setting reference df.")
                reference = res_df
...
def single_metric_exp(df, exclude_list, is_numeric, trans_dict):
    ...
    for metric in metric_to_sort_by:
        start = time.time()
        Anova_dict, MI_dict, emb_sim_dict, std_dict, reg_weight_dict, multi_atom_combinations, bucket_objects, num_groups_over100, num_groups_over500, hdb_dict = setup_cherrypicking(
            df, exclude_list, is_numeric, trans_dict, start, do_regression=(metric == DF_PVALUE_STRING), compute_hypdb=False)
...
def pred_level_cherrypicking(df, exclude_list, is_numeric, trans_dict):
    start = time.time()
    Anova_dict, MI_dict, emb_sim_dict, std_dict, reg_weight_dict, multi_atom_combinations, bucket_objects, num_groups_over100, num_groups_over500, hdb_dict = setup_cherrypicking(
        df, exclude_list, is_numeric, trans_dict, start, do_regression=False, compute_hypdb=False)
...
def run_sample_guided_experiment(sample_sizes, iterations, df, exclude_list, is_numeric, trans_dict, max_atoms):
        ...
        for i in range(iterations):
            ...
            if USE_SQL:
                cherry_picker = CherryPicker(exclude_list=exclude_list, numeric_list=is_numeric,
                                             ...
                                             std_dict=std_dict,
                                             output_path=output_path, start_time=start_time, stop_at_time=STOP_AT_TIME,
                                             metric_subset=METRICS_SUBSET, max_atoms=max_atoms)
            sorted_by = sort_attr_tuples(multi_atom_combinations, SORT_BY, Anova_dict, MI_dict, emb_sim_dict, std_dict,
                                         reg_weight_dict, num_groups_over100, hdb_dict)
...
def sensitivity_to_k(k_values, df, exclude_list, is_numeric, trans_dict, reference_path=None):
    ...
    for top_k in k_values:
        ...
        Anova_dict, MI_dict, emb_sim_dict, std_dict, reg_weight_dict, multi_atom_combinations, bucket_objects, num_groups_over100, num_groups_over500, hdb_dict = setup_cherrypicking(
            df, exclude_list, is_numeric, trans_dict, start_time, do_regression=True, compute_hypdb=False)
        print("Cherrypicking is starting.")
        if USE_SQL:
            cherry_picker = CherryPicker(exclude_list=exclude_list, numeric_list=is_numeric,
                                         grp_attr=GRP_ATTR, target_attr=TARGET_ATTR,
...
def run_multiple_methods_for_query(target_attr, grp_attr, compare_list, agg_type,
    ...
    query_desc = f"{grp_attr}_{compare_list[2]}_gt_{compare_list[1]}".replace(" ", "_")
    reference = None
    for method in methods:
