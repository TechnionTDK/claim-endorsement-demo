...
def create_column_dictionary_for_SO(fields_list):
    updated_dict = {
        ...
        "YearsCode_disc": "Years Code",
        "YearsCode_Pro_disc": "Years Code Professionally",
        "WorkExp_disc": "Work Experience",
        "ICorPM": "Independent Contributor or People Manager",
    }
    for column in fields_list:
        if column not in updated_dict.keys():
            updated_dict[column] = ' '.join(re.findall('[A-Z][^A-Z]*', column))
    for col in ['Gender', 'Trans', 'Ethnicity', 'Accessibility', 'MentalHealth']:
        updated_dict[(col, 'Or, in your own words:')] = 'Other (user specified)'
    return updated_dict


def make_translation_for_flights(fields_list):
    if fields_list is None:
        fields_list = ['YEAR', 'MONTH', 'DAY', 'DAY_OF_WEEK', 'AIRLINE', 'FLIGHT_NUMBER', 'TAIL_NUMBER',
                       'ORIGIN_AIRPORT_CODE',
                       ...
                       'ORIGIN_AIRPORT', 'ORIGIN_CITY', 'ORIGIN_STATE', 'ORIGIN_COUNTRY', 'ORIGIN_LATITUDE',
                       'ORIGIN_LONGITUDE',
                       'DEST_AIRPORT', 'DEST_CITY', 'DEST_STATE', 'DEST_COUNTRY', 'DEST_LATITUDE', 'DEST_LONGITUDE',
                       'DELAYED']
    trans_dict = {c: c.replace("_", " ").lower() for c in fields_list}
    trans_dict['distance_disc'] = "distance"
    weekdays = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"]
    weekday_d = {("DAY_OF_WEEK", i+1): weekdays[i] for i in range(7)}
    trans_dict.update(weekday_d)
    trans_dict[('DIVERTED', 0)] = "No"
    trans_dict[('DIVERTED', 1)] = "Yes"
    airline_code_to_name = pd.read_csv("data/flights/airlines.csv", index_col=0)['AIRLINE'].to_dict()
    for k in airline_code_to_name:
        trans_dict[("AIRLINE", k)] = airline_code_to_name[k]
    return trans_dict
...
def check_ACS_fields():
    fields_list = ['RT', 'SERIALNO', 'DIVISION', 'SPORDER', 'PUMA', 'REGION', 'ST', 'ADJINC', 'PWGTP', 'AGEP', 'CIT',
                   'CITWP', 'COW', 'DDRS', 'DEAR', 'DEYE', 'DOUT', 'DPHY', 'DRAT', 'DRATX', 'DREM', 'ENG', 'FER', 'GCL',
                   'GCM', 'GCR', 'HINS1', 'HINS2', 'HINS3', 'HINS4', 'HINS5', 'HINS6', 'HINS7', 'INTP', 'JWMNP',
                   'JWRIP', 'JWTR', 'LANX', 'MAR', 'MARHD', 'MARHM', 'MARHT', 'MARHW', 'MARHYP', 'MIG', 'MIL', 'MLPA',
                   'MLPB', 'MLPCD', 'MLPE', 'MLPFG', 'MLPH', 'MLPI', 'MLPJ', 'MLPK', 'NWAB', 'NWAV', 'NWLA', 'NWLK',
                   'NWRE', 'OIP', 'PAP', 'RELP', 'RETP', 'SCH', 'SCHG', 'SCHL', 'SEMP', 'SEX', 'SSIP', 'SSP', 'WAGP',
                   'WKHP', 'WKL', 'WKW', 'WRK', 'YOEP', 'ANC', 'ANC1P', 'ANC2P', 'DECADE', 'DIS', 'DRIVESP', 'ESP',
                   'ESR', 'FOD1P', 'FOD2P', 'HICOV', 'HISP', 'INDP', 'JWAP', 'JWDP', 'LANP', 'MIGPUMA', 'MIGSP', 'MSP',
                   'NAICSP', 'NATIVITY', 'NOP', 'OC', 'OCCP', 'PAOC', 'PERNP', 'PINCP', 'POBP', 'POVPIP', 'POWPUMA',
                   'POWSP', 'PRIVCOV', 'PUBCOV', 'QTRBIR', 'RAC1P', 'RAC2P', 'RAC3P', 'RACAIAN', 'RACASN', 'RACBLK',
                   'RACNH', 'RACNUM', 'RACPI', 'RACSOR', 'RACWHT', 'RC', 'SCIENGP', 'SCIENGRLP', 'SFN', 'SFR', 'SOCP',
                   'VPS', 'WAOB', 'FAGEP', 'FANCP', 'FCITP', 'FCITWP', 'FCOWP', 'FDDRSP', 'FDEARP', 'FDEYEP', 'FDISP',
                   'FDOUTP', 'FDPHYP', 'FDRATP', 'FDRATXP', 'FDREMP', 'FENGP', 'FESRP', 'FFERP', 'FFODP', 'FGCLP',
                   'FGCMP', 'FGCRP', 'FHICOVP', 'FHINS1P', 'FHINS2P', 'FHINS3C', 'FHINS3P', 'FHINS4C', 'FHINS4P',
                   'FHINS5C', 'FHINS5P', 'FHINS6P', 'FHINS7P', 'FHISP', 'FINDP', 'FINTP', 'FJWDP', 'FJWMNP', 'FJWRIP',
                   'FJWTRP', 'FLANP', 'FLANXP', 'FMARP', 'FMARHDP', 'FMARHMP', 'FMARHTP', 'FMARHWP', 'FMARHYP', 'FMIGP',
                   'FMIGSP', 'FMILPP', 'FMILSP', 'FOCCP', 'FOIP', 'FPAP', 'FPERNP', 'FPINCP', 'FPOBP', 'FPOWSP',
                   'FPRIVCOVP', 'FPUBCOVP', 'FRACP', 'FRELP', 'FRETP', 'FSCHGP', 'FSCHLP', 'FSCHP', 'FSEMP', 'FSEXP',
                   'FSSIP', 'FSSP', 'FWAGP', 'FWKHP', 'FWKLP', 'FWKWP', 'FWRKP', 'FYOEP', 'PWGTP1', 'PWGTP2', 'PWGTP3',
