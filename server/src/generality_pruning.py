...
def prune_by_generality(res_df, max_atoms, is_oreo_format=False):
    # 1. get the set of attrs in each row, and size of set
    if is_oreo_format:
        res_df['Attr_set'] = res_df['Counter argument'].apply(lambda atom_list: set([x.split(":")[0] for x in atom_list]))
    if not is_oreo_format:
