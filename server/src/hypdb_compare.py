...
def compute_covariates_and_responsibility(df, grp_attr, value1, value2, target_attr, potential):
    start = time.time()
    df = df[df[grp_attr].isin([value1, value2])]
    if grp_attr not in potential:
        cols = potential + [grp_attr]
    else:
        cols = potential
    df1 = df[cols]  # This should not contain the outcome, but it should contain the grp_attr
    h1 = cs.hypdb(data=df1)

    cov1, par1 = h1.get_parents([grp_attr], pvalue=0.01, method='g2', ratio=1, fraction=1, num_samples=1000,
                                blacklist=[""], whitelist=[], debug=False, coutious='yes',
                                loc_num_samples=100, maxmc=1000, fullcheck=True, optimized=False)
    h = cs.hypdb(data=df)
    cov_list = h.recommend_covarite(treatment=grp_attr, outcome=target_attr, potential=cov1)
    resp = matching.get_respon(df, treatment=[grp_attr], outcome=target_attr, covariates=cov_list)
    end = time.time()
    print(f"Covariate analysis took {end - start} seconds")
    return resp


def compare_ACS_f_m_pincp(grp_attr, value1, value2, target_attr):
    # grp_attr= 'SEX'
    ...
    resp = compute_covariates_and_responsibility(df, grp_attr, value1, value2, target_attr, potential)
    return resp


def compare_SO_salary(grp_attr, value1, value2, target_attr):
...
if __name__ == '__main__':
    ...
    # BSC vs MSC
    print(compare_SO_salary('EdLevel', 'Master’s degree', 'Bachelor’s degree', 'ConvertedCompYearly'))
